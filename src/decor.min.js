...
Decor.Scene=function(a,c){function b(){if(!n){n=!0;e.$[0].scrollLeft=0;f();for(var a=0;a<c.objects.length;a++){var b=c.objects[a];b.o=b.o||{};if(!b.o.noIE||!isIE){var g,k,m;for(m in b){g=m;k=b[m];break}c.prototypes&&/^\$/.test(k)&&(k=k.substr(1),b.o=$.extend(b.o,c.prototypes[k]),k=k.toLowerCase());b.o.img&&r++;var h=Decor.Things[g];if(h){var l=e,p=k,b=(b=b.o)||{};b.dims=b.dims||[1,1];b.pos=b.pos||[0,0,0];b.rot=b.rot||[0,0,0];b=new h(l,p,b);b.name=k;e.objects.push(b)}else console.warn("Object type "+
g+" not found")}}if(c.oninit)c.oninit(e);r||d()}}function d(){e.$.addClass("loaded animate");e.show();c.scrollLeft&&setTimeout(function(){e.$[0].scrollLeft=Math.round(10*e.width*c.scrollLeft)})}function f(){clearTimeout(q);q=setTimeout(g,20);e.$.addClass("resizing");p||(p=e.$[0].scrollLeft/e.$[0].scrollWidth);var a=[innerWidth,innerWidth/h];a[1]>innerHeight&&(a=[innerHeight*h,innerHeight]);var b={"margin-left":e.margin[0]=c.fullWidth?0:Math.round((innerWidth-a[0])/2),"margin-top":e.margin[1]=Math.round((innerHeight-
a[1])/2)};c.fixedSize&&c.res?e.$.css(c3.transform,"scale("+(e.scale=a[0]/k[0])+")"):(e.width=Math.round(a[0]),b.width=c.fullWidth?innerWidth:e.width,b.height=e.height=Math.round(a[1]));e.$.css(b).trigger("scene-resize")}function g(){e.$.removeClass("resizing");p&&(e.$[0].scrollLeft=Math.round(p*e.$[0].scrollWidth));p=0}function l(a){for(var b in e.objects)e.objects[b].destroy&&e.objects[b].destroy();e.$.remove();delete e.$;e.deleted=!0;a&&a()}c=$.extend(!0,{},c);var e=this,h=16/9,k=c.res||[innerWidth,
innerHeight*h],m=!1,n=!1,r=0,p=0,q=null;c.width=c.width||1;c.height=c.height||1;this.name=a;this.data=c;this.width=k[0];this.height=k[1];this.scale=1;this.objects=[];this.collisionObjects=[];this.margin=[0,0];this.camera=new Decor.Camera(this);this.deleted=this.active=!1;this.$=$("<scene>").addClass(a).css({width:k[0],height:k[1]}).appendTo(document.body);c.scrolling&&1<c.width&&$('<div class="canvas">').css("width",100*c.width+"%").appendTo(this.$);this.imageLoaded=function(){--r||d()};this.addLoadImage=
function(){r++};this.getThing=function(a){for(var b in e.objects)if(e.objects[b].name==a)return e.objects[b]};this.show=function(){if(!m){if(n)f();else return b();Decor.currentScene&&Decor.delete(Decor.currentScene.name);Decor.currentScene=this;localStorage.setItem("currentScene",a);e.$.addClass("placed");m=!0;e.active=!0;addEventListener("resize",f);c.audio&&Decor.Audio.play(c.audio.src,c.audio);e.$.trigger("scene-show",a);setTimeout(function(){e.$.addClass("shown");$(document.body).addClass("scene-shown")})}};
this.hide=function(b){if(!m)return b&&b();m=!1;e.active=!1;e.$.removeClass("shown");$(document.body).removeClass("scene-shown");$window.add(e.$).trigger("scene-hide",a);removeEventListener("resize",f);c.audio&&Decor.Audio.stop(c.audio.src);setTimeout(function(){e.$.removeClass("placed");b&&b()},1E3)};this.delete=function(a){m?e.hide(function(){l(a)}):l(a)}};
Decor.Camera=function(a){function c(c){var d={l:b.position[0]-1,r:b.position[0]+2,t:b.position[1]-0.5,b:b.position[1]+1},e;for(e in a.objects){var f=a.objects[e];f.matrix&&(f.attr.main||(a.data.renderAll||f.matrix.intersects(d))&&c&&c.call(f))}}var b=this,d=a.data.height-1,f="0,0,0";this.offset=[0,d,0];this.position=[0,0,0];this.setPosition=function(a,l){var e=a+"";f!=e&&(f=e,b.position[0]=Math.max(0,a[0]),b.position[1]=Math.min(d,Math.max(0,a[1])),b.position[2]=-a[2],c(function(){this.place(l)}))};
...
c.hasClass("open"))return $(".nav.back").click();var f="zoomout";if(c.toggleClass("open").hasClass("open")){f="zoomin";a.$.find(".open").not(c).removeClass("open");var g=d.matrix.getPosition(),l=d.$.width(),e=d.$.height();e>a.height?g[2]-=a.height-1.05*e:l>a.width&&(g[2]-=(a.width-l)/2);g[0]-=(1-b.width)/2;a.camera.setPosition(g)}else a.camera.reset();a.$.trigger(f,d.name)};this.place();a.$.on("scene-resize",d.place)};
Decor.Mat3D=function(a,c,b,d){c=c||[0,0,0];b=b||[0,0,0];d=d?d.length?d:[d||1,d||1]:[1,1];var f=this,g=a.scene,l=g.camera.position,e=[0,0,0],h=[a.rwidth,a.rheight];this.getQuad=function(a){a=a||[0,0,0];var b=f.getPosition();return{l:b[0]+a[0],r:b[0]+a[0]+h[0],t:-(b[1]-a[1]),b:-(b[1]-a[1]-h[1])}};this.intersects=function(a){var b=f.getQuad();return!(a.l>b.r||a.r<b.l||a.t>b.b||a.b<b.t)};this.getCSS=function(f){var h=a.scene.camera.offset,n=a.attr.relative?0:l[0],r=a.attr.relative?0:h[1]-(g.data.height-
1)-l[1],p=[g.width,g.height],q=[];f||(n-=g.$[0].scrollLeft/g.width);f=[Math.round(p[0]*(c[0]+e[0]+h[0]-n)),-Math.round(p[1]*(c[1]+e[1]+r)),Math.round(0.3*(c[2]+e[2]+l[2]))+h[2]];q.push("translate3d("+f.join("px,")+"px)");b[0]&&q.push("rotateX("+b[0]+"deg)");b[1]&&q.push("rotateY("+b[1]+"deg)");b[2]&&q.push("rotateZ("+b[2]+"deg)");"1,1"!=String(d)&&q.push("scale3d("+d.join(",")+",1)");return q.join(" ")};this.getPosition=function(a){return[c[0]+e[0],-(c[1]+e[1]),c[2]+e[2]]};this.getElementPosition=
function(){var b=f.getPosition(),c=getComputedStyle(a.$[0])[c3.transform];/^matrix\(/.test(c)&&(c=c.substr(0,c.length-1).split(","),b[0]+=parseFloat(c[4])/g.width,b[1]+=parseFloat(c[5])/g.height);return b};this.getX=function(){return c[0]+e[0]};this.getY=function(){return c[1]+e[1]};this.getZ=function(){return c[2]+e[2]+l[2]};this.translate=function(a,b){b&&(e=[0,0,0]);a&&(e[0]+=a[0],e[1]+=a[1],e[2]+=a[2]);return f}};
...
Decor.Things.ImageContain=function(a,c,b){function d(){var a=h.height*e.width()/h.width,b=Math.round(a*n[0][1]);k.css({height:a-b,bottom:b})}b.width=b.width||1;b.dims=[b.width,0];var f=this,g=/\..{3}$/.test(b.img)?"":".png",g="img/"+b.img+g,l=b.px?b.px[0]:100*b.dims[0]+"%",e=$("<dud>").css("width",l).appendTo(a.$);b.class&&e.addClass(b.class);this.attr=b;this.scene=a;var h=null,h=new Image;h.thing=f;h.onload=function(g){b.dims[1]=b.dims[0]*h.height/h.width;f.rwidth=b.dims[0];f.rheight=b.dims[1];m&&
d();f.$.addClass("image "+c);b.static&&(f.$.css("width",l),f.$cnt.replaceWith(f.$),e=f.$);Decor.Object3D.call(f,a,e,b);b.clickable&&(m?k:f.$).click(f.focus);b.static||f.$.appendTo(f.$cnt);if(f.onload)f.onload();a.imageLoaded(c)};h.onerror=a.imageLoaded;h.src=g;f.$=$(h);f.$cnt=e;e[0].thing=this;var k=$(),m=b.clickable&&b.clickable.length,n=b.clickable&&b.clickable.length?b.clickable:[[0,0],[1,1]];b.clickable&&(e.addClass("clickable"),m&&(e.addClass("has-area"),k=$('<div class="click">').css({left:100*
n[0][0]+"%",width:100*(n[1][0]-n[0][0])+"%"}).appendTo(e)));if(m)a.$.on("scene-resize",d)};Decor.Things.ImageRep=function(a,c,b){a.imageLoaded();for(var d=0;d<b.repeat;d++)a.addLoadImage(),a.objects.push(new Decor.Things.Image(a,c,{img:b.img,width:b.width,pos:[b.pos[0]+d*b.width,b.pos[1],b.pos[2]]}))};
